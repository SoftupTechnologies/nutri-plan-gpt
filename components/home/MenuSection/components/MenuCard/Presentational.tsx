...
const MenuCard: React.FC<MenuCardProps> = (props) => {
  ...
  return (
    <>
      ...
      />
      <aside
        className="cursor-pointer flex h-[100px] w-[500px]] max-w-xl rounded-[30px] bg-white p-1 shadow items-center"
        style={{ background: '#A9EDD8' }}
        onClick={() => setShowMealModal(true)}
      >
        <div className="flex text-white flex-col p-2" style={{ flex: 0.7 }}>
          <h1 className="pb-4 text-lg font-semibold text-black">
            {data.mealName}
          </h1>
        ...
        <div style={{ flex: 0.3 }}>
          <span style={{ backgroundColor: '#6BDAB1' }} className='text-sm  p-2 rounded-2xl font-bold'>Get cooking</span>
        </div>
      </aside>
    </>
  );
...
export const timeee = (<svg width="17" height="21" viewBox="0 0 17 21" fill="none" xmlns="http://www.w3.org/2000/svg">
